...
## Supported Export Quality

By *default*, FusionExport generates output files (exported charts) that are of `better` quality, which is reasonable for most situations. However, there are situations when:

* File size is the main constraint, and output quality is a secondary concern. In such cases, you can use the `good` quality to suit your needs.

* The output file has to be of highest quality, and file size is not a limitation. In such cases, you can use the `best` quality.
