...
> Replace the map definition files (of v3.12.2 or older) with the latest files available in [download package](https://cdn.fusioncharts.com/downloads/addons/fusionmaps-xt-definition.zip) while upgrading to __v3.13.0__.
...
The FusionCharts Suite XT download package contains only the USA and World maps as part of the package. To work with other maps, you need the definition files for them. A map definition file contains all the data necessary to create and render a map. You can download the additional map definition files from [here](https://cdn.fusioncharts.com/downloads/addons/fusionmaps-xt-definition.zip).

Note that the definition files you download will yield maps in extremely high resolutions. If you do not need such high-resolution maps, send in a [request for custom resolution maps](https://cdn.fusioncharts.com/downloads/addons/fusionmaps-xt-definition.zip) to our Customer Support team.
