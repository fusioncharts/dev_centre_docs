...
`Pivot` is an operator which converts one tabular expression into another - more specifically, from row to column level. It is used when you want to change the unique values from one column in the expression to multiple columns to render a chart. Applying `Pivot` to a `DataTable` transforms the grouping of a particular column into multiple columns.
...
This argument is the same as of groupBy method. It is used to arrange identical data into groups of a table. In the first argument, each group of configuration object can have the following attributes:
...
This attribute is only applicable if the grouping is applied on date/time. This attribute specifies how the date column should be grouped. For example, if `timeUnit` is set to **Month**, the grouping will be applied month wise. The **month** is one of the `DatetimeUnits` enum and not any random string.
...
`outputFormat` attribute is used to set the format of the column being grouped. To set the date/time format, set the value of `outputFormat` attribute as per your requirement.
...
In the above code, `binSize` attribute has been set to **10** which means starting from **0** grouping will take place up to 10th value.

### Pivot Column

New `DataTable` was created when the groupBy method was applied to the master `DataTable`. Now, let's apply `Pivot` on a particular column of the new `DataTable`. Pivot is applied to the column with which you want to group the grouped `DataTable`.

### Aggregation Configuration

In the third argument of the groupBy method, the following attributes can be applied to every group of configuration:
